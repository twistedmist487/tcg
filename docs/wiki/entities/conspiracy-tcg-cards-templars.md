---
title: Conspiracy TCG — Templar cards
created: 2026-08-15
updated: 2026-08-15
type: entity
tags: [project, gaming, conspiracy, status]
sources: []
confidence: high
contested: false
---

# Conspiracy TCG — Templar cards

Taunt, heals, Shielding, holy removal.

Energy type: **Faith**. Source of truth: data/cards.json in the [[conspiracy-tcg]] repo.

| | Count |
|---|---:|
| Characters | 18 |
| Spells | 14 |
| Locations | 8 |
| **Total** | **40** |

| Name | Type | Cost | ATK | HP | Effect |
|---|---|---:|---:|---:|---|
| Novice | Character | 1 | 1 | 2 | Shielding. |
| Squire | Character | 1 | 1 | 3 | Taunt. |
| Archivist | Character | 2 | 1 | 3 | When played, draw a card. When this character dies, draw a card. |
| Battle Chaplain | Character | 2 | 2 | 2 | When played, restore 3 Health to your hero. |
| Templar Guardian | Character | 2 | 1 | 3 | Taunt (Enemy characters must attack this character if able). |
| Field Medic | Character | 3 | 2 | 3 | Assault: Restore 2 Health to a target. |
| Relic Keeper | Character | 3 | 2 | 3 | Taunt. When this character is damaged, deal 1 damage to a random enemy character. |
| Zealot | Character | 3 | 3 | 2 | Charge. When this character dies, deal 2 damage to the enemy hero. |
| Exorcist | Character | 4 | 3 | 3 | Destroyed enemy characters cannot be resurrected or returned to hand. |
| Knight Commander | Character | 4 | 3 | 5 | Other Templar characters you control gain +1 Health. |
| Vanguard | Character | 4 | 3 | 3 | Charge. |
| Warden of Secrets | Character | 4 | 2 | 5 | Taunt. When this character is destroyed, draw 2 cards. |
| Crusader | Character | 5 | 4 | 4 | Charge (Can attack the turn it is played). |
| Healing Seraph | Character | 5 | 3 | 5 | At the end of your turn, restore 2 Health to all friendly characters. |
| Crypt Warden | Character | 6 | 3 | 5 | Taunt. Recur. |
| Paladin Champion | Character | 6 | 4 | 6 | Other Templar characters you control gain +1 Attack. |
| Temple Sentinel | Character | 6 | 4 | 7 | Taunt. When this character is damaged, deal 2 damage to a random enemy character. |
| Grand Master Aldric | Character | 9 | 6 | 7 | Taunt. Holy. At the start of your turn, restore 3 Health to all your characters. |
| Baptistry | Location | 3 | — | — | At the start of your turn, restore 1 Health to your hero. |
| Monastery Archives | Location | 3 | — | — | At the start of your turn, if you have fewer cards than your opponent, draw a card. |
| Blessed Armory | Location | 4 | — | — | Your characters gain +1 Attack when they attack. |
| Sacred Chapel | Location | 4 | — | — | At the start of your turn, heal 1 damage from all your characters. |
| Holy Grail Sanctum | Location | 5 | — | — | At the start of your turn, if you have a damaged character, restore 2 Health to it. |
| Reliquary | Location | 5 | — | — | At the start of your turn, heal 1 damage from all your characters. |
| Fortress Citadel | Location | 6 | — | — | Your characters gain +1 Health. At the end of your turn, restore 1 Health to your hero. |
| Cathedral Spire | Location | 7 | — | — | At the start of your turn, restore 1 Health to all friendly characters and draw 1 card. |
| Absolution | Spell | 1 | — | — | Restore 5 Health to your hero or a character. Draw a card. |
| Anoint | Spell | 2 | — | — | Give a friendly character Shielding. |
| Shield Wall | Spell | 2 | — | — | Give all your characters +2 Health until end of turn. |
| Consecration | Spell | 3 | — | — | Deal 1 damage to all enemy characters. |
| Divine Smite | Spell | 3 | — | — | Deal 4 damage to a target character. |
| Holy Ground | Spell | 3 | — | — | All your characters gain Taunt until end of turn. Restore 2 Health to your hero. |
| Divine Favor | Spell | 4 | — | — | Give a friendly character +3 Attack and 'Can't be targeted by enemy abilities' until end of turn. |
| Martyr's Blessing | Spell | 4 | — | — | Destroy a friendly character. Draw 3 cards. Restore 5 Health to your hero. |
| Penance | Spell | 4 | — | — | Deal 3 damage to a target character. Restore 2 Health to your hero. |
| Excommunicate | Spell | 5 | — | — | Deal 4 damage to a target character. |
| Holy Inquisition | Spell | 5 | — | — | Silence and deal 3 damage to an enemy character. |
| Judgment | Spell | 6 | — | — | Deal 5 damage to an enemy character. Restore 5 Health to your hero. |
| Benediction | Spell | 7 | — | — | Restore 5 Health to your hero. Draw 2 cards. |
| Wrath of God | Spell | 7 | — | — | Deal 2 damage to all enemy characters. Restore 2 Health to all friendly characters. |

## Related

- [[conspiracy-tcg]] — project overview
- [[conspiracy-tcg-cards]] — all lists
- [[chris]] — owner
